...
function initGlobalDelegation() {
    document.addEventListener('click', (e) => {
        ...
        // Checkout - Primary (redirect to checkout page)
        const checkoutBtn = e.target.closest('.btn-checkout-primary');
        if (checkoutBtn) { goToCheckout(); return; }

        // Checkout - WhatsApp (legacy)
        const checkoutWhatsApp = e.target.closest('.btn-checkout-whatsapp');
        if (checkoutWhatsApp) { checkout(); return; }

        // Legacy: old btn-checkout without specific class
        const legacyCheckoutBtn = e.target.closest('.btn-checkout:not(.btn-checkout-primary)');
        if (legacyCheckoutBtn && !legacyCheckoutBtn.classList.contains('btn-checkout-whatsapp')) { goToCheckout(); return; }
...
function renderCartItems() {
    ...
    footer.innerHTML =
        ...
        '</div>' +
        '<button class="btn-checkout btn-checkout-primary">' +
        '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="1" y="4" width="22" height="16" rx="2" ry="2"/><line x1="1" y1="10" x2="23" y2="10"/></svg>' +
        'Proceder al pago' +
        '</button>' +
        '<button class="btn-checkout-whatsapp" data-action="whatsapp">' +
...
function goToCheckout() {
    ...
    // Close cart sidebar and redirect
